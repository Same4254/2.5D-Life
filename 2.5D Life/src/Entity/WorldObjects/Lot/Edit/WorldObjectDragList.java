...
//	private ArrayList<WorldObject> dragList;
	...
	public void fill(AXIS axisToFill, Vector2f position) {
		System.out.println("Fill Pos: " + position);
		if(original != null) {
//			if(axisToFill == AXIS.X_AXIS) {
//				if(Math.abs(position.distance(original.getPosition2D())) < original.getWidth() * .5) {
//					clear(AXIS.X_AXIS);
//					clear(AXIS.Z_AXIS);
//					return;
//				}
//			} else if(axisToFill == AXIS.Z_AXIS) {
//				if(Math.abs(position.distance(original.getPosition2D())) < original.getHeight() * .5) {
//					clear(AXIS.X_AXIS);
//					clear(AXIS.Z_AXIS);
//					return;
//				}
				...
				for(int i = 0; i <= Math.abs(position.x - original.getX()) / original.getWidth(); i++) {
					if(i >= xAxis.size()) {
						...
						if(position.x < original.getX()) 
							temp.setPosition2D(new Vector2f(original.getX() - (original.getWidth() * (i + 1)), original.getZ()).subtract(original.getWidth() / 2, original.getHeight() / 2));
						else
							temp.setPosition2D(new Vector2f(original.getX() + (original.getWidth() * (i + 1)), original.getZ()).subtract(original.getWidth() / 2, original.getHeight() / 2));
						continue;
					}
				...
				for(int i = 0; i <= Math.abs(position.y - original.getZ()) / original.getHeight(); i++) {
					if(i >= zAxis.size()) {
						...
						if(position.y < original.getZ()) 
							temp.setPosition2D(new Vector2f(original.getX(), original.getZ() - (original.getHeight() * (i + 1))).subtract(original.getWidth() / 2, original.getHeight() / 2));
						else
							temp.setPosition2D(new Vector2f(original.getX(), original.getZ() + (original.getHeight() * (i + 1))).subtract(original.getWidth() / 2, original.getHeight() / 2));
						continue;
					}
