...
public class GoToBelowFloorAction extends MultiAction {
	...
	public void start() {
		...
		if(entity.getPosition3D().y < Lot.FLOOR_HEIGHT) {
			complete = true;
			return;
		}
		
		Lot lot = handler.getWorld().getLot(entity.getPosition2D());
		Floor floor = lot.getFloor(entity.getPosition3D().subtract(0, Lot.FLOOR_HEIGHT, 0));
		...
		if(front.x > 0) {
			...
			subActions.add(new MovementFunction(handler, entity, MovementFunction.stairMovement, floor.getPosition().y, floor.getPosition().y + Lot.FLOOR_HEIGHT, false, false, true));
		} else if(front.y < 0) {
			subActions.add(new GoToAction(handler, lot, entity, stair.getPosition2D().add(0, stair.getHeight())));
			subActions.add(new MovementFunction(handler, entity, MovementFunction.stairMovement, floor.getPosition().y, floor.getPosition().y + Lot.FLOOR_HEIGHT, false, true, true));
		} else if(front.x < 0) {
			subActions.add(new GoToAction(handler, lot, entity, stair.getPosition2D().add(stair.getWidth(), 0)));
			subActions.add(new MovementFunction(handler, entity, MovementFunction.stairMovement, floor.getPosition().y, floor.getPosition().y + Lot.FLOOR_HEIGHT, true, false, true));
		} else if(front.y > 0) {
			subActions.add(new GoToAction(handler, lot, entity, stair.getPosition2D()));
			subActions.add(new MovementFunction(handler, entity, MovementFunction.stairMovement, floor.getPosition().y, floor.getPosition().y + Lot.FLOOR_HEIGHT, false, true, true));
		}
