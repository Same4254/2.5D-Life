...
public class GoToNextFloorAction extends MultiAction {
	...
	public void start() {
		...
		if(front.x > 0)
			subActions.add(new MovementFunction(handler, entity, MovementFunction.stairMovement, floor.getPosition().y, floor.getPosition().y + Lot.FLOOR_HEIGHT, true, false, false));
		else if(front.y < 0)
			subActions.add(new MovementFunction(handler, entity, MovementFunction.stairMovement, floor.getPosition().y, floor.getPosition().y + Lot.FLOOR_HEIGHT, false, true, false));
		else if(front.x < 0)
			subActions.add(new MovementFunction(handler, entity, MovementFunction.stairMovement, floor.getPosition().y, floor.getPosition().y + Lot.FLOOR_HEIGHT, false, false, false));
		else if(front.y > 0)
			subActions.add(new MovementFunction(handler, entity, MovementFunction.stairMovement, floor.getPosition().y, floor.getPosition().y + Lot.FLOOR_HEIGHT, true, true, false));
